-- Keep catalog declarations aligned with docs/postgresql-compatibility.md.
...
AS 'uuid_eq' LANGUAGE internal IMMUTABLE STRICT LEAKPROOF PARALLEL SAFE;
...
AS 'uuid_ne' LANGUAGE internal IMMUTABLE STRICT LEAKPROOF PARALLEL SAFE;
CREATE OPERATOR <> (LEFTARG = compact_uuid, RIGHTARG = compact_uuid,
...
AS 'uuid_lt' LANGUAGE internal IMMUTABLE STRICT LEAKPROOF PARALLEL SAFE;
CREATE OPERATOR < (LEFTARG = compact_uuid, RIGHTARG = compact_uuid,
...
AS 'uuid_le' LANGUAGE internal IMMUTABLE STRICT LEAKPROOF PARALLEL SAFE;
CREATE OPERATOR <= (LEFTARG = compact_uuid, RIGHTARG = compact_uuid,
...
AS 'uuid_gt' LANGUAGE internal IMMUTABLE STRICT LEAKPROOF PARALLEL SAFE;
CREATE OPERATOR > (LEFTARG = compact_uuid, RIGHTARG = compact_uuid,
...
AS 'uuid_ge' LANGUAGE internal IMMUTABLE STRICT LEAKPROOF PARALLEL SAFE;
CREATE OPERATOR >= (LEFTARG = compact_uuid, RIGHTARG = compact_uuid,
...
AS 'uuid_cmp' LANGUAGE internal IMMUTABLE STRICT LEAKPROOF PARALLEL SAFE;
...
CREATE FUNCTION compact_uuid_equalimage(oid) RETURNS boolean
AS 'btequalimage' LANGUAGE internal IMMUTABLE STRICT PARALLEL SAFE;
CREATE FUNCTION compact_uuid_skipsupport(internal) RETURNS void
AS 'uuid_skipsupport' LANGUAGE internal IMMUTABLE STRICT PARALLEL SAFE;
...
USING btree FAMILY pg_catalog.uuid_ops AS
    ...
    FUNCTION 4 compact_uuid_equalimage(oid),
    FUNCTION 6 compact_uuid_skipsupport(internal);
CREATE OPERATOR CLASS compact_uuid_ops DEFAULT FOR TYPE compact_uuid
